...
private:
	...
	std::size_t otherDoor_(std::size_t door1, std::size_t door2);
...
public:
	...
	MontyHall();
	~MontyHall() = default;
	bool game(MontyHall::Strategies st);
	std::size_t gameSample(std::size_t n, MontyHall::Strategies st);
};

inline std::size_t MontyHall::otherDoor_(std::size_t door1, std::size_t door2)
{
	std::size_t other = 0;
	if (door1 != door2)
	{
...
inline bool MontyHall::game(MontyHall::Strategies st)
{
	...
	const std::size_t secondChoice =
		(st == MontyHall::Strategies::KEEP) ? firstChoice : otherDoor_(openDoor, firstChoice);

	return secondChoice == prizeDoor;
}

inline std::size_t MontyHall::gameSample(std::size_t n, MontyHall::Strategies st)
{
